...
local function copy_webkit_files()
    ...
    local src_js = fs.join(public_dir, "RolacoTools.js")
    local dst_js = fs.join(target_webkit_dir, "RolacoTools.js")
    ...
    local src_luxury_css = fs.join(public_dir, "luxury.css")
    local dst_luxury_css = fs.join(target_webkit_dir, "luxury.css")
    if fs.exists(src_luxury_css) then
        local content = m_utils.read_file(src_luxury_css)
        if content then m_utils.write_file(dst_luxury_css, content) end
    end
end

local function inject_webkit_files()
    millennium.add_browser_css("webkit/steamdb-webkit.css")
    millennium.add_browser_css("webkit/luxury.css")
    millennium.add_browser_js("webkit/RolacoTools.js")
end

-- ── Lifecycle ────────────────────────────────────────────────────────────────

local function on_load()
    logger.log("Bootstrapping RolacoTools plugin, millennium " .. millennium.version())
...
local function on_unload()
    logger.log("unloading RolacoTools plugin")
end
...
function FixNoConnection()
    local ok, res = pcall(auto_update.fix_no_connection)
    if not ok then
        logger.warn("FixNoConnection failed: " .. tostring(res))
        return json_err(res)
    end
    return json_ok(res)
end
...
function HasRolacoToolsForApp(appid)
...
function StartAddViaRolacoTools(appid)
    if type(appid) == "table" then appid = appid.appid end
    local ok, res = pcall(downloads.start_add_via_RolacoTools, tonumber(appid))
    if not ok then return json_err(res) end
    return json_ok(res)
end

function GetAddViaRolacoToolsStatus(appid)
    if type(appid) == "table" then appid = appid.appid end
...
function CancelAddViaRolacoTools(appid)
...
function StartAddViaRolacoToolsFromUrl(apiName, appid, contentScriptQuery, url)
    ...
    logger.log("StartAddViaRolacoToolsFromUrl CALLED: appid=" ..
    tostring(appid) .. ", url=" .. tostring(url) .. ", apiName=" .. tostring(apiName))

    local ok, res = pcall(downloads.start_add_via_RolacoTools_from_url, appid, url, apiName)
    if not ok then
        logger.warn("StartAddViaRolacoToolsFromUrl CRASHED inside pcall: " .. tostring(res))
        return json_err(res)
    end
...
function GetIconDataUrl()
    ...
    local icon_path = fs.join(paths.get_plugin_dir(), "public", "RolacoTools-icon.png")
...
function DeleteRolacoToolsForApp(appid)
    if type(appid) == "table" then appid = appid.appid end
