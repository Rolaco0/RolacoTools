...
local function _launch_async_download(appid, url, dest_path, extract_dir)
    ...
    if is_windows then
        local cmd = string.format(
            'cmd.exe /C start "RolacoTools Downloader" cmd.exe /C "color 0B && echo RolacoTools is downloading the requested files... && echo Please keep this window open until it closes automatically. && echo. && (echo {"status": "downloading"} > "%s" && curl.exe -# -L -A "discord(dot)gg/mGU77b9X7" "%s" -o "%s" && echo {"status": "extracting"} > "%s" && echo. && echo Extracting files... && tar.exe -xf "%s" -C "%s" && echo {"status": "extracted"} > "%s") || (echo. && echo ERROR: Download or extraction failed! && echo {"status": "failed"} > "%s" && timeout /t 5)"',
