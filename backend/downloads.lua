...
local function _launch_async_download(appid, url, dest_path, extract_dir)
    ...
    if is_windows then
        local cmd = string.format(
            'cmd.exe /C start "RolacoTools Downloader" cmd.exe /C "color 0B && echo RolacoTools is downloading the requested files... && echo Please keep this window open until it closes automatically. && echo. && (echo {"status": "downloading"} > "%s" && curl.exe -# -L -A "discord(dot)gg/RolacoTools" "%s" -o "%s" && echo {"status": "extracting"} > "%s" && echo. && echo Extracting files... && tar.exe -xf "%s" -C "%s" && echo {"status": "extracted"} > "%s") || (echo. && echo ERROR: Download or extraction failed! && echo {"status": "failed"} > "%s" && timeout /t 5)"',
...
function downloads.start_add_via_RolacoTools_from_url(appid, url, apiName)
    if type(appid) == "string" then appid = tonumber(appid) end
    if not appid then return { success = false, error = "Invalid appid" } end

    logger.log("RolacoTools: StartAddViaRolacoToolsFromUrl appid=" .. tostring(appid) .. " api=" .. tostring(apiName))
    ...
    if not ok then
        logger.warn("RolacoTools: Async Download crashed - " .. tostring(res))
        _set_download_state(appid, { status = "failed", error = tostring(res) })
...
function downloads.start_add_via_RolacoTools(appid)
    if type(appid) == "string" then appid = tonumber(appid) end
    if not appid then return { success = false, error = "Invalid appid" } end

    logger.log("RolacoTools: StartAddViaRolacoTools appid=" .. tostring(appid))
    ...
    if not ok then
        logger.warn("RolacoTools: start_add_via_RolacoTools crashed - " .. tostring(res))
        _set_download_state(appid, { status = "failed", error = tostring(res) })
        return { success = false, error = tostring(res) }
    end
