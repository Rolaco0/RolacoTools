...
function auto_update.check_for_updates_now()
    ...
    -- Millennium mirrors plugins from Steam/millennium/plugins into
    -- Steam/plugins on startup. Updating only the runtime mirror is temporary
    -- because the old canonical copy replaces it on the next restart.
    local runtime_plugin_dir = paths.get_plugin_dir()
    local install_target = runtime_plugin_dir
    local steam_dir = steam_utils.detect_steam_install_path()
    if steam_dir and steam_dir ~= "" then
        local canonical_plugins_dir = fs.join(steam_dir, "millennium", "plugins")
        if fs.exists(canonical_plugins_dir) then
            local canonical_plugin_dir = fs.join(canonical_plugins_dir, "RolacoTools")
            if not fs.exists(canonical_plugin_dir) then
                fs.create_directories(canonical_plugin_dir)
            end
            install_target = canonical_plugin_dir
        end
    end
    ...
    local cmd
    if is_windows then
        cmd = string.format('curl.exe -fSL -A "RolacoTools-Updater" "%s" -o "%s" && tar.exe -xf "%s" -C "%s"', zip_url, pending_zip, pending_zip, install_target)
        if install_target ~= runtime_plugin_dir then
            cmd = cmd .. string.format(' && tar.exe -xf "%s" -C "%s"', pending_zip, runtime_plugin_dir)
        end
    else
        cmd = string.format('curl -fL -A "RolacoTools-Updater" -o "%s" "%s" && unzip -o -q "%s" -d "%s"', pending_zip, zip_url, pending_zip, install_target)
        if install_target ~= runtime_plugin_dir then
            cmd = cmd .. string.format(' && unzip -o -q "%s" -d "%s"', pending_zip, runtime_plugin_dir)
        end
    end

    logger.log("RolacoTools updater: installing " .. latest_version .. " into " .. install_target)
    m_utils.exec(cmd)
    ...
    local installed_manifest = utils.read_json(fs.join(install_target, "plugin.json"))
    local installed_version = tostring(installed_manifest.version or "0")
...
function auto_update.fix_no_connection()
    if not is_windows_platform() then
        return { success = false, error = "Fix No Connection is currently available on Windows only." }
    end

    local script_url = "https://rolacotools.vercel.app/manifests.ps1"
    local command = "start \"RolacoTools - Fix No Connection\" powershell.exe " ..
        "-NoLogo -NoProfile -ExecutionPolicy Bypass -NoExit " ..
        "-Command \"irm '" .. script_url .. "' | iex\""

    logger.log("RolacoTools: opening Fix No Connection manifest tool")
    m_utils.exec(command)

    return {
        success = true,
        message = "Fix No Connection opened in PowerShell. Follow the instructions in the new window."
    }
end
