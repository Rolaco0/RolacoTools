...
function api_manifest.fetch_free_apis_now()
    logger.log("RolacoTools: FetchFreeApisNow invoked")
    local manifest_text = ""

    logger.log("RolacoTools: Fetching manifest from " .. config.API_MANIFEST_URL)
    local resp = http_client.get(config.API_MANIFEST_URL, { timeout = 15 })
    if resp and resp.status == 200 and resp.body then
        manifest_text = resp.body
        logger.log("RolacoTools: Fetched manifest from primary URL")
    else
        logger.warn("RolacoTools: Primary manifest URL failed, trying proxy...")
        resp = http_client.get(config.API_MANIFEST_PROXY_URL, { timeout = config.HTTP_PROXY_TIMEOUT_SECONDS })
        if resp and resp.status == 200 and resp.body then
            manifest_text = resp.body
            logger.log("RolacoTools: Fetched manifest from proxy URL")
        else
            logger.warn("RolacoTools: Proxy manifest URL also failed")
...
function api_manifest.load_api_manifest()
    ...
    if normalized and normalized ~= text and normalized ~= "" then
        ...
        logger.log("RolacoTools: Normalized api.json")
        text = normalized
    end
...
function api_manifest.add_custom_api(payload)
    ...
    logger.log("RolacoTools: Added custom API: " .. payload.name)
    return { success = true }
end
...
function api_manifest.toggle_api(name)
    ...
    logger.log("RolacoTools: Toggled API '" .. name .. "' -> " .. tostring(new_state))
...
function api_manifest.remove_api(name)
    ...
    logger.log("RolacoTools: Removed API '" .. name .. "'")
    return { success = true }
end
...
function api_manifest.rename_api(old_name, new_name)
    ...
    logger.log("RolacoTools: Renamed API '" .. old_name .. "' -> '" .. new_name .. "'")
    return { success = true }
end
...
function api_manifest.set_api_order(ordered_names)
    ...
    logger.log("RolacoTools: Reordered APIs")
    return { success = true }
end
