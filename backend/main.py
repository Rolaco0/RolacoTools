...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying RolacoTools web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy RolacoTools web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied RolacoTools icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy RolacoTools icon: {exc}")
    else:
        logger.warn(f"RolacoTools icon not found at {icon_src}")
...
def _inject_webkit_files() -> None:
    ...
    logger.log(f"RolacoTools injected web UI: {js_path}")
...
def HasRolacoToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_RolacoTools_for_app(appid)


def StartAddViaRolacoTools(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_RolacoTools(appid)


def GetAddViaRolacoToolsStatus(appid: int, contentScriptQuery: str = "") -> str:
...
def CancelAddViaRolacoTools(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_RolacoTools(appid)
...
def GetMorrenusStats(api_key: str, force_refresh: bool = False, contentScriptQuery: str = "", **kwargs: Any) -> str:
    ...
    try:
        ...
        client = ensure_http_client("RolacoTools: GetMorrenusStats")
        ...
        logger.warn(f"RolacoTools: GetMorrenusStats failed: {exc}")
        return json.dumps({"error": str(exc)})


def StartAddViaRolacoToolsFromUrl(appid: int, url: str, apiName: str, contentScriptQuery: str = "") -> str:
    return start_add_via_RolacoTools_from_url(appid, url, apiName)
...
def DeleteRolacoToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_RolacoTools_for_app(appid)
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"RolacoTools: OpenExternalUrl failed: {exc}")
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"RolacoTools: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
    ...
    try:
        ...
        if os.path.exists(themes_path):
            try:
                ...
                logger.warn(f"RolacoTools: Failed to read themes.json: {exc}")
                return json.dumps({"success": False, "error": "Failed to read themes.json"})
        else:
            return json.dumps({"success": True, "themes": []})
    except Exception as exc:
        logger.warn(f"RolacoTools: GetThemes failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "RolacoTools: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"RolacoTools: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("RolacoTools: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("RolacoTools: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("RolacoTools: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("RolacoTools: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            else:
                payload = changes
        ...
        if payload is None:
            ...
            logger.warn(f"RolacoTools: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"RolacoTools: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"RolacoTools: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"RolacoTools: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"RolacoTools: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"RolacoTools: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"RolacoTools: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableThemes(contentScriptQuery: str = "") -> str:
    ...
    try:
        ...
        logger.warn(f"RolacoTools: GetAvailableThemes failed: {exc}")
...
class Plugin:
    ...
    def _load(self):
        logger.log(f"bootstrapping RolacoTools plugin, millennium {Millennium.version()}")

        try:
            detect_steam_install_path()
        except Exception as exc:
            logger.warn(f"RolacoTools: steam path detection failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"RolacoTools: settings initialization failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"RolacoTools: Fixes index initialization failed: {exc}")
